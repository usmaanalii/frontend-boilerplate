...
gulp.task('html-partial', function() {
    return gulp.src([htmlFiles]).pipe(htmlPartial({basePath: 'src/html/partials/'})).pipe(gulp.dest(htmlDest)).pipe(browserSync.reload({stream: true}));
});
...
gulp.task('sass-compile', function() {
    ...
    return gulp.src('src/sass/main.sass').pipe(sourcemaps.init()).pipe(plumber({errorHandler: onError})).pipe(bulkSass()).pipe(sass({includePaths: ['src/sass']})).pipe(autoprefixer({browsers: ['last 2 versions'], cascade: false})).pipe(rename('main.min.css')).pipe(cleanCSS()).pipe(gulp.dest(cssDest)).pipe(browserSync.reload({stream: true}));
});

gulp.task('css-purify', function() {
    console.log('Removing unused CSS');
    return gulp.src('dist/css/main.min.css').pipe(purify([jsFiles, htmlFiles])).pipe(cleanCSS()).pipe(gulp.dest(cssDest));
});
...
// Babel transform
bundler.transform(babelify.configure({sourceMapRelative: 'app/js'}));
...
function bundle() {
    ...
    return bundler.bundle().on('error', function(err) {
        ...
        this.emit("end");
    }).pipe(exorcist('.dist/js/bundle.js.map')).pipe(source('bundle.js')).pipe(gulp.dest('./dist/js')).pipe(browserSync.stream({once: true}));
}
...
gulp.task('js-bundle-minify', function() {
    ...
    return gulp.src('dist/js/bundle.js').pipe(uglify()).pipe(gulp.dest('dist/js'));
});
...
gulp.task('sprite-generator', function() {
    ...
    var spriteData = gulp.src('dist/img/*.png').pipe(spritesmith({imgName: 'sprite.png', cssName: '_sprite-sheet.sass'}));
    ...
    var cssStream = spriteData.css.pipe(gulp.dest('src/sass/helpers'));
    ...
    var makeSpriteSheet = file('_sprite.sass', spriteSass.spriteImport + '\n\n' + spriteSass.spriteInclude, {src: true}).pipe(gulp.dest('src/sass/helpers'));
...
gulp.task('img-min', function() {
    ...
    // Caching images that ran through imagemin
        .pipe(cache(imagemin({verbose: true, interlaced: true}))).pipe(gulp.dest('dist/img'));
});

gulp.task('img-resize', function() {
    console.log('Reducing image to specified size');
    gulp.src('dist/img/image-2.png').pipe(imageResize({width: 200, height: 200, crop: true, upscale: false})).pipe(gulp.dest('dist/img'));
});
/**
 ****************************************************
